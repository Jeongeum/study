...
const searchCoordinateToAddress = (latlng, map) => {
  ...
  naver.maps.Service.reverseGeocode(
    ...
    (status, response) => {
      ...
      const address = response.v2.address.readAddress
...
const searchAddressToCoordinate = (address, map) => {
  naver.maps.Service.geocode({ query: address }, (status, response) => {
    ...
    const address = item.readAddress ? item.roadAddress : item.jibunAddress;
...
function App() {
  ...
  const onClickHandler = (e) => {
    const inputVaule = e.target.querySelector('input')?.value;
  ...
  return (
    <div>
      ...
      <form onSubmit={onSubmitHandler}>
        <input type="text" placeholder="(예:제주시)" />
